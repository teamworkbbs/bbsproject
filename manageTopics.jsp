...
<script type="text/javascript">
	...
	/*
	* function top() {
		if (confirm("此操作将置顶，你确认要进行操作吗？")) {
			return true;
		}
		return false;
	}
	function untop() {
		if (confirm("此操作将取消置顶，你确认要进行操作吗？")) {
			return true;
		}
		return false;
	}*/
			...
			<table class="topicStyle" cellspacing="0">
				...
				<s:iterator value="listTopic" var="topic">
					<tr style="height: 26px;">
						...
						<td><s:if test="#topic.Top==0">否</s:if> <s:else>是</s:else></td>
						...
						<td style="color: blue;"><s:if test="#topic.niceTopic==0">
							...
							<s:if test="#topic.topTopic==0">
								<a style="color: red" onclick="return top()"
									href="topic_Managetop.action?topic.id=<s:property
								value="#topic.id" />">置顶</a>
							</s:if> <s:else>
								<a onclick="return untop()"
									href="topic_ManageUntop.action?topic.id=<s:property
								value="#topic.id" />">取消置顶</a>
							</s:else>&nbsp;&nbsp;
