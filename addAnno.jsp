...
<%
	...
	if (user == null) {
		...
		pw.println("<script type='text/javascript'>alert('您未登录或登录已失效！请登录！');window.location.href = '"+basePath+"login.jsp';</script>");

	} else if (user.getRoleId() == 0) {
		PrintWriter pw = response.getWriter();
		pw.println("<script type='text/javascript'>alert('您的权限不够！请切换账号登录');window.location.href = '"+basePath+"login.jsp';</script>");
	} else {
%>
<title>发布公告</title>
